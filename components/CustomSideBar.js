...
export default class CustomSideBarMenu extends Component{
  ...
  uploadImage=async(uri, imageName)=>{
    ...
    var ref=firebase.storage().ref().child("user_pofilles/"+imageName);
    return ref.put(blob).then((response)=>{
      this.fetchImage(imageName);
    }) 
  }
  fetchImage=(imageName)=>{
    var storageRef=firebase.storage().ref().child("user_pofilles/"+imageName);
  ...
  render(){
    return(
      <View style={{flex:1}}>
        <View style={{flex:0.5, allignItems:"center", backgroundColor:"orange"}}>
...
var styles = StyleSheet.create({
  ...
  imageContainer: 
  { flex: 0.75, width: "40%", height: "20%", marginLeft: 20, marginTop: 30, borderRadius: 40, }
})
